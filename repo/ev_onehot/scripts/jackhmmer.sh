...
## Jackhmmer search homology sequences (target)
jackhmmer -N $niter \
...
python $cur_sh_dir/tblout2ids.py $tblout $targetidfile

## Fetch homology sequences
fastafile="$dir/target_seqs.fasta"
txtfile="$dir/target_seqs.txt"
esl-sfetch -o $fastafile -f $seqdb $targetidfile
python $cur_sh_dir/fasta2txt.py $fastafile $txtfile

## Create alignment file
python $cur_sh_dir/sto2a2m.py $query $alignmentfile ${dir}/alignment
for (( i=1; i<=$niter; i++ ))
do
    python $cur_sh_dir/sto2a2m.py $query $aliprefix-$i.sto $aliprefix-$i
done
